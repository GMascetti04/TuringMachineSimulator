...
import { TapeWriter } from '../TM';
export enum MachineSpeed {
    STEP_BY_STEP = 0,
    SLOW = 1,
    NORMAL = 2,
    FAST = 3,
...
function toString(s: MachineSpeed): string {
    switch (s) {
...
function isInputStringValid(inputAlphabet: string[], input: string): string | undefined {
...
function InputParameterComponent({ setMachineInput, inputAlphabet, allowEditing, compute }:
    { setMachineInput: (t: string) => void, inputAlphabet: string[], allowEditing: boolean, compute: (input: string, maxTrans: number, speed: MachineSpeed) => void }) {

    const [curInput, changeCurInput] = React.useState<string>('');
    const [curInputErrorMsg, changeCurInputErrorMsg] = React.useState<string | undefined>(undefined);

    const [curMachineSpeed, changeCurMachineSpeed] = React.useState<MachineSpeed>(MachineSpeed.STEP_BY_STEP);

    const [curMaxTransitionsText, changeCurMaxTransitionsText] = React.useState<string>('100');
